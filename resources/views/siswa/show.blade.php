...
<div class="row">
    ...
    <div class="col-12 col-md-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Transaksi Daftar Ulang <span class="tag {{ $daful->where('lunas',1)->count() > 0 ? 'tag-success':'tag-warning' }}">
                        @if($daful->where('lunas', 1)->count() > 0)
                        Lunas
                        @elseif($daful->sum('jumlah_bayar') == 0)
                        Belum Bayar
                        @else
                        Belum Lunas (-{{$kurang}})
                        @endif
            ...
            <div class="card-body">
                ...
                <table class="table card-table table-hover table-vcenter text-wrap">
                    ...
                    @foreach($daful as $item)
                    <tr>
                        <td>{{ $item->created_at->format('d-m-Y') }}</td>
                        <td>
                            {{$item->siswa->kelas->periode->nama}}
                        </td>
                        <td style="min-width: 100px">IDR. {{ format_idr($item->jumlah_bayar) }}</td>
                    </tr>
                    @endforeach
                </table>
                @else
