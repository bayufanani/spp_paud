                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <div class="form-group" style="display: none" id="form-bulan">
                            ...
                            @for($i=1;$i<=12;$i++) <input type="checkbox" name="bulans[]" value="{{$i}}"> {{bulan_indo($i)}} <span style="width:20px; display: inline-block"></span>
                                @endfor
                        </div>
    ...
    require(['jquery', 'select2', 'sweetalert'], function($, select2, sweetalert) {
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        for (i = 0; i < result.length; i++) {
                            var bulans = '';
                            if (result[i].transaksi.length > 0) {
                                bulans = result[i].transaksi[0].bulan;
                            }
            ...
            $('#tagihan').on('change', function() {
                ...
                // set bulan terselect
                var blns = bulan.split(',');
                var checkbox_bln = $('#form-bulan > input[type="checkbox"]');
                checkbox_bln.toArray().forEach(cb => {
                    cb.checked = false;
                });

                if (blns != '') {
                    blns.forEach(bln => {
                        checkbox_bln[parseInt(bln) - 1].checked = true;
                        checkbox_bln[parseInt(bln) - 1].disabled = true;
                    });
                }
            ...
            $('#btn-simpan').on('click', function() {
                ...
                if ((harga - diskon) == NaN) {
                    ...
                    // handle penyimpanan bulan
                    var bulan_selected = [];
                    var checkbox_bln = $('#form-bulan > input[type="checkbox"]');
                    checkbox_bln.toArray().forEach((cb, id) => {
                        if (cb.checked == true) {
                            bulan_selected.push(id + 1);
                        }
                    });
